...
In this paper, we will not discuss \textit{Fors} nor \textit{Alloy} in details. We assume that the reader has a general background about the idea of those two projects. For your convenience we included a quick reference in the appendix.\\
\end{comment}
...
We will discuss the solutions and the related work in the next chapters.
...
Forgery contains a main package and three additional packages. In total, it has thirteen modules as illustrated in figure 5.2. The Main module is responsible for starting the program and for exporting the SQL data in the end of the process. The parser converts the input into AST objects that are used by the database schema and procedures generator. Map modules are responsible to map between different objects, simplify the data structure and to extract different types of data.
...
\Function{Generate Predicate Procedures}{$predicates$}
	...
	\State \textbf{add} SQL Statement ("DECLARE EXIT HANDLER FOR SQLEXCEPTION; BEGIN; ROLLBACK; END; START TRANSACTION;") \textbf{to} $query$
		...
		\For {$l$ in $p.body$} \Comment Operations over tables
			...
			\ElsIf{$l.algebraicOperation$ is $!=$} \State \textbf{add} SQL Statement ("IF (CHECKSUM TABLE $l.op1$, $l.op2$) IS NOT NULL THEN (SELECT 'precondition violated'; ROLLBACK);") \textbf{to} $query$
...
\label{array-sum4}
\begin{algorithmic}[1]
\Function{Generate Invariant Procedures}{$facts$, $quantifiers$}
	\State $query = ""$
	\For {$fq$ in ${facts+quantifiers}$}
		...
		\If{$fq$ is $fact$} 
			\For {$l$ in $fq.body$} \Comment Operations over tables
				\State $tmpQuery = ""$
				\State \textbf{l = Simplify tuples and joins (l)}
				\If{$l.algebraicOperation$ is $in$} \State \textbf{add} SQL Statement ("SELECT * FROM $l.op2$ WHERE id NOT IN (SELECT ${l.op2}_id$ FROM $l.op1$)'") \textbf{to} $tmpQuery$
			
				\ElsIf{$l.algebraicOperation$ is $not in$} \State \textbf{add} SQL Statement ("SELECT * FROM $l.op2$ WHERE id IN (SELECT ${l.op2}_id$ FROM $l.op1$)'") \textbf{to} $tmpQuery$
			
				\ElsIf{$l.algebraicOperation$ is $=$} \State \textbf{add} SQL Statement ("(CHECKSUM TABLE $l.op1$, $l.op2$) IS NULL") \textbf{to} $tmpQuery$
				
				\ElsIf{$l.algebraicOperation$ is $!=$} \State \textbf{add} SQL Statement ("(CHECKSUM TABLE $l.op1$, $l.op2$) IS NOT NULL") \textbf{to} $tmpQuery$
			
				\EndIf
			\EndFor
			\If{$fq.variables$} \State \textbf{extend} SQL Statement ("SELECT * FROM (tmpQuery)") \textbf{to} $query$
		...
		\Comment Context of the quantifier
			\If{$fq.context$ is $signature$} \State \textbf{extend} SQL Statement ("SELECT * FROM fq.tableName") \textbf{to} $query$
			\ElsIf{$fq.type$ is $field$} 
...
\label{array-sum5}
\begin{algorithmic}[1]
\Function{Generate Quantifiers}{$q$}
	\State $query = ""$
	\If{$q.type$ is $lone$} \State \textbf{extend} SQL Statement ("HAVING COUNT('id') > 1") \textbf{to} $query$
	\ElsIf{$q.type$ is $one$} 
		\State \textbf{extend} SQL Statement ("HAVING COUNT('id') <> 1") \textbf{to} $query$
	\ElsIf{$q.type$ is $some$} 
		\State \textbf{extend} SQL Statement ("HAVING COUNT('id') <= 1") \textbf{to} $query$
	\ElsIf{$q.type$ is $no$} 
...
We executed the same operations in Forgery's database schema and by using its procedures ($p\_enroll$, $create\_course$, $create\_student$).  The output was similar.
...
Simulating those four scenarios in Forgery for different inputs will support us finding bugs in Forgery, and fix them so it will better match to Alloy's behavior. Currently, the process itself is not fully automatic, but we used multiple inputs and compared them in order to cover as many scenarios as possible.
...
\begin{comment} 
// TODO: ADD TO BIB
// can be used for the intro etc
An abstract Alloy specification is far from an actual implementation, and manual implementation is not easy. refining the former into the latter is unfortunately a non-trivial task.
\end{comment}
