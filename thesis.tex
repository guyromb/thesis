...
Let $q$ be a quantifier, containing its type, its context and its restriction domain.
	...
	\For {$fq$ in ${facts+quantifiers}$}
		...
		\Comment Context of the quantifier
			...
			\ElsIf{$fq.type$ is $field$} 
				\State \textbf{extend} SQL Statement ("SELECT * FROM fq.tableName LEFT JOIN fq.field ON fq.fieldId='fq.tableName'.'id'")\textbf{to} $query$
			\EndIf
			\Comment Type of the quantifier
			\If{$fq.type$ is $lone$} \State \textbf{extend} SQL Statement ("HAVING COUNT('id') > 1") \textbf{to} $query$
			\ElsIf{$fq.type$ is $one$} 
				\State \State \textbf{extend} SQL Statement ("HAVING COUNT('id') <> 1") \textbf{to} $query$
			\ElsIf{$fq.type$ is $some$} 
				\State \State \textbf{extend} SQL Statement ("HAVING COUNT('id') <= 1") \textbf{to} $query$
			\ElsIf{$fq.type$ is $no$} 
				\State \State \textbf{extend} SQL Statement ("HAVING COUNT('id') > 0") \textbf{to} $query$
			\EndIf
		\EndIf
